...
void Proposal::executeImpl(const float *input0, const float *input1, std::vector<size_t> dims0,
    ...
    // Unpacked boxes buffers and is_dead buffers combined to achieve alignment along the buffer start.
    ...
    for (int n = 0; n < nn; ++n) {
        ...
        if (nms_kernel_) {
            ...
            nms_cpu(pre_nms_topn, is_dead, &unpacked_boxes[0], roi_indices, &num_rois, 0, conf.nms_thresh_,
